...
\apiitem{bool {\ce split_sample} (int pixel, int sample, float zsplit)}
\NEW % 1.8
If the specified sample's depth range spans {\cf zsplit}, split it into two
samples with depth ranges $[z,\mathrm{zsplit})$ and
$[\mathrm{zsplit},\mathrm{zback})$ with appropriate changes to their color
and alpha values so that their net transparency is the same as before.
Return true if a split occurred, false if the sample was not modified.
\apiend

\apiitem{int {\ce split} (int pixel, float zsplit)}
Split any samples of the pixel whose depth range spans {\cf zsplit}.
Return the number of splits that occurred.
\apiend

\apiitem{int {\ce split_all} (int pixel, const DeepData \&src, int srcpixel,\\
        \bitspc   bool do_split=true) \\
int {\ce split_all} (int pixel, bool do_split=true)}
\NEW % 1.8
Split all samples in the indexed pixel by all the sample edges in {\cf
src}'s {\cf srcpixel}, returning the number of splits (i.e., the number of
additional samples that were added). If {\cf do_split} is false, don't
actually perform any splits or modify the pixel, just return the number of
splits that would have been performed.

For the version that does not specify a second image/pixel, it means to
split the pixel in this image by all its own sample edges.
\apiend

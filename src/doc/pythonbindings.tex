...
\apiitem{ROI {\ce roi_union} (roiA, roiB) \\
ROI {\ce roi_intersection} (roiA, roiB)}
Return the union, or intersection, respectively, of two ROI's.
\apiend

\apiitem{ROI {\ce parse_roi} (geom, roibase=ROI(), allow_scaling=True)}
Parse the string {\cf geom} for either a WxY+X+Y style descrition or a
XBEGIN,XEND,YBEGIN,YEND range-style description (and both, for 3D).
\apiend
...
\apiitem{bool DeepData.{\ce split_sample} (pixel, sample, zsplit)}
Split the given sample at the depth {\cf zsplit} (if the sample does in fact
span that depth). Return {\cf True} if a split occurred, {\cf False} if
the sample was unmodified.
\apiend

\apiitem{int DeepData.{\ce split} (pixel, zsplit)}
Split all samples of the pixel that span {\cf zsplit}. Return the number
of splits that occurred.
\apiend

\apiitem{int DeepData.{\ce split_all} (pixel, src, srcpixel, do_split=True)}
\NEW % 1.8
Split all samples of the pixel that edges of a pixel of DeepData {\cf src}.
Return the number of splits that occurred. If {\cf do_split} is {\cf False},
the pixel will not be altered, but it will still return the number of splits
...
\apiitem{ImageBuf ImageBufAlgo.{\ce deep_cull} (src, holdout, roi=ROI.All, nthreads=0) \\
bool ImageBufAlgo.{\ce deep_cull} (dst, src, holdout, roi=ROI.All, nthreads=0)}
...
\begin{code}
    ...
    Holdout = ImageBuf("holdout.exr")
    Result = ImageBuf()
    ImageBufAlgo.deep_cull (Result, Img, Holdout)
\end{code}
\apiend

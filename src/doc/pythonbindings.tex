...
{\cf float} (scaling all channels) or both tuples giving a {\cf float} for
each color channel.
...
\apiitem{bool ImageBufAlgo.{\ce macc} (dst, A, B, C, roi=ROI.All, nthreads=0)}
\index{ImageBufAlgo!macc} \indexapi{macc}
\NEW % 1.8

Compute {\cf dst} = {\cf dst + A * B} (channel-by-channel multiplication and
accumulation). {\cf A} is an \ImageBuf, and {\cf B} and may be either a
{\cf float} (scaling all channels) or a tuple giving a {\cf float} value for
each color channel.

\smallskip
\noindent Example:
\begin{code}
    # Compute a weighted sum of a.exr and b.exr
    buf = ImageBuf ()
    ImageBufAlgo.macc (buf, ImageBuf("a.exr"), Aweight)
    ImageBufAlgo.macc (buf, ImageBuf("b.exr"), Bweight)
\end{code}
\apiend
